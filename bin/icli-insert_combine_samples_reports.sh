...
#
# Syntax:
#  icli_insert_combine-samples-reports <run_id> <file>
...
# Remove percentage sign to store data as numeric
sed -i s/"%"//g $1.tmp
...
# Add to DB
PGPASSWORD=$dbpassword psql -U ${dbuser} -d 'i2' -t -c "COPY combine_samples_report FROM STDIN  DELIMITER E'	' NULL 'null' CSV;" < report-tmp.csv
